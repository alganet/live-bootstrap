...
to_builder_hex0_src () {
    ...
    while read -r fs_entry; do
        ...
        if [ -d "$fs_entry" ]; then
            ...
            printf %s\\n "src ${wc_out%% *} ${prefix}${fs_entry#\.\/mirror}"
            $CAT "$fs_entry"
        fi
...
# Creates ./target/init.builder-hex0-src
{
    ...
    walk_manifest_distfiles echo | to_builder_hex0_src "/external/distfiles"
...
$DD if=/dev/zero of="target/init.img" bs=512 count=16777216 # 8GB (standard GitHub runner has 14GB disk space)
...
HTTPMIRROR_PID=$!
# Launch image (standard GH runner has 4 cores, 16GB RAM)
printf %s\\n "Starting $QEMU..." 1>&2
$QEMU \
    --enable-kvm \
    -serial file:output \
    ...
    -machine kernel-irqchip=split >/dev/null &
QEMU_PID=$!

printf %s\\n "Watching $QEMU logs..."

while [ -d /proc/$QEMU_PID ]
do
    sleep 60
    printf '[1min watch] Log Lines: %s - Last 3:\n' "$(wc -l output | cut -d" " -f1)"
    tail -n4 output | head -n3
done
